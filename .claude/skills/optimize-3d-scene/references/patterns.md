...
Every block here is production code from shipped animation-heavy sites, reduced
to its generalised form. The three.js/WebGL parts are framework-independent —
**adapt the framework glue, keep the reasoning comments**, they are why the
numbers are what they are. Where a block shows framework-specific wiring
(a client-only leaf, a server-side bot check), swap it for the equivalent in
this project's stack — see `.claude/stack.json`.
...
### Plain / vanilla — no framework

```ts
// Three WebGL scenes each running their own forever-rAF is a classic cause of
// whole-page scroll jank. Only render while the section is on (or near) screen.
let raf = 0;
let running = false;
...
### The client leaf that wires it together (React shown; same shape in any stack)
...
**Audit dead chains.** A common inherited shape is three chained composers
where two render empty layers and the final pass
...
The correct shape, as used for scroll-revealed logo particles:
...
light (compute the fresnel *instead of* the fills — the mistake is shipping both, three real
lights on top of it):
...
```ts
// Keep the decoder LOCAL (served from the project's static root). A CDN round-trip on
...
```ts
// Read the request's user-agent through whatever server API this stack exposes:
// Next `headers()`, SvelteKit `event.request.headers`, Nuxt `useRequestHeaders`,
// Astro `Astro.request.headers`, a plain Node `req.headers`. Static-only stacks
// cannot do this at all — use the reduced-motion path instead.

export const isBot = async (getUserAgent: () => Promise<string | null>): Promise<boolean> => {
  const ua = ((await getUserAgent()) || "").toLowerCase();
  return (
