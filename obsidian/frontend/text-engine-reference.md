---
tags: [frontend, motion, reference, stable, stack-specific]
updated: 2026-08-18
---

# TextEngine — Full Reference

> Complete API reference for `spring-text-engine`. **React stacks only** — it is
> the `bindings.textMotion` default there. On any other framework, ignore this
> note and follow the recipe in [[text-motion]], which reproduces the same
> behaviour and carries the same three traps.
>
> For the rules and traps that apply either way, see [[text-motion]] — this note
> is the API surface, not the conventions.
...
| `overflow` | `boolean` | `false` | Sets `overflow: hidden` on wrapLine / wrapWord. **Requires leading ≥ 1.1** (`leading-display`) or glyphs clip — see [[text-motion#The three traps]] |
